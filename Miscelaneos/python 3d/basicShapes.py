...
def FractalLandscape(origin=(0,0,0), dimensions=(400,400), iterations=4, height=40):
    ...
    #------------------------------------------------------------------------------ 
    #    Punto medio entre dos o mas nodos
    ...
    for i in range(iterations):
        ...
        for (n1, n2) in edges:
            nodes.append(midpoint([nodes[n1], nodes[n2]]))                                          # Add nodes midway between each edge
        ...
        nodes.sort(key=lambda node: (node[2],node[0]))                                              # Sort in order of grid

        size = size*2-1
                
        edges = [(x+y*size, x+y*size+1) for y in range(size) for x in range(size-1)]                # Horizontal edge
        edges.extend([(x+y*size, x+(y+1)*size) for x in range(size) for y in range(size-1)])        # Vertical edges
    ...
    grid = wf.Wireframe(nodes)      # Crea vértices al modelo
    grid.addEdges(edges)            # A�ade lados al modelo
